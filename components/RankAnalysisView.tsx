    ...
    return (
        <div className="w-full py-4 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
                <DashboardTable title={`Top 1 ${displayMode === 'daily' ? '日均' : '最高分'}`} data={top1List} columns={[{ header: '#', className: 'w-10' }, { header: '活動 (Event)' }, { header: '分數 (Score)', className: 'text-right' }]} renderRow={(s, idx) => renderEventRow(s, idx, getValue(s, s.top1))} color="bg-yellow-500" />
                <DashboardTable title={`Top 10 ${displayMode === 'daily' ? '日均' : '最高分'}`} data={top10List} columns={[{ header: '#', className: 'w-10' }, { header: '活動 (Event)' }, { header: '分數 (Score)', className: 'text-right' }]} renderRow={(s, idx) => renderEventRow(s, idx, getValue(s, s.top10))} color="bg-purple-500" />
                <DashboardTable title={`Top 100 ${displayMode === 'daily' ? '日均' : '最高分'}`} data={top100List} columns={[{ header: '#', className: 'w-10' }, { header: '活動 (Event)' }, { header: '分數 (Score)', className: 'text-right' }]} renderRow={(s, idx) => renderEventRow(s, idx, getValue(s, s.top100))} color="bg-cyan-500" />
                {/* Fix: Changed undefined renderRow call to renderEventRow */}
                <DashboardTable title={`Top ${selectedBorderRank}`} headerAction={<Select value={selectedBorderRank} onChange={(val) => setSelectedBorderRank(Number(val))} onClick={(e) => e.stopPropagation()} className="text-xs py-1" options={BORDER_OPTIONS.map(rank => ({ value: rank, label: `T${rank}` }))} />} data={borderRankList} columns={[{ header: '#', className: 'w-10' }, { header: '活動 (Event)' }, { header: '分數 (Score)', className: 'text-right' }]} renderRow={(s, idx) => renderEventRow(s, idx, getValue(s, s.borders[selectedBorderRank] || 0))} color="bg-teal-500" />
            </div>
        </div>
    );
};
