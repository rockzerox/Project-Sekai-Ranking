...
interface SidebarProps {
  currentView: 'home' | 'live' | 'past' | 'comparison' | 'analysis' | 'worldLink' | 'playerAnalysis';
  setCurrentView: (view: 'home' | 'live' | 'past' | 'comparison' | 'analysis' | 'worldLink' | 'playerAnalysis') => void;
  isOpen: boolean;
  toggleSidebar: () => void;
  isCollapsed: boolean;
  toggleCollapse: () => void;
    ...
    <>
      ...
      >
        ...
        <nav className="p-4 space-y-2 flex-1 custom-scrollbar overflow-y-auto">
          ...
          {/* 4. Rank Analysis (Renamed) */}
          <button onClick={() => { setCurrentView('analysis'); if (window.innerWidth < 768) toggleSidebar(); }} title="活動榜線分析" className={itemClass('analysis')}>
            <svg className="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
            {!isCollapsed && <span className="whitespace-nowrap overflow-hidden">活動榜線分析</span>}
          </button>

          {/* 5. World Link */}
          <button onClick={() => { setCurrentView('worldLink'); if (window.innerWidth < 768) toggleSidebar(); }} title="World Link Analysis" className={itemClass('worldLink')}>
            <svg className="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
