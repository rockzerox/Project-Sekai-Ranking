...
*   **World Link 章節切換**: 若當前活動為 World Link 類型，支援切換「總榜」與各角色的「個人章節」排行榜。具備獨立的章節時間感知，未開始與結算中的章節會有對應的狀態與倒數提示。
...
*   **分頁與章節解耦**: 在 Top 100 模式下支援分頁瀏覽；在精彩片段模式下顯示關鍵名次。切換排行榜分頁或排序時，當前選擇的 World Link 章節會被保留，不會自動重置回總榜。
