...
*   **手機版優化**: 在手機版（≤640px）上，圖表中的角色名稱預設隱藏，以最大化長條圖顯示空間，懸停時會顯示角色名稱。
*   **手機端 Compact Mode**: 手機版自動啟用緊湊模式（由 `useMobile` 偵測），Bar 高度縮減為 `h-5`（20px）、頭像縮為 `w-5 h-5`、行間距收緊為 `gap-1`，使單一視窗可顯示約 20~22 條角色。
...
*   `src/hooks/useMobile.ts` (手機端斷點偵測)
